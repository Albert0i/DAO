...
The use of Lua script is meant to cut down round tips. Both versions bear similar logic. 
...
Redis is in-memory datastore featuring data structures such as `string`, `list`, `set`, `sorted set`, `hash`... etc, but there no point to implement our own full-text search by reading hash by hash, checking value by value. Although it is completely possible but is considered not effective and violates [DRY](https://en.wikipedia.org/wiki/Don%27t_repeat_yourself) principle after all. 
...
Currently, RediSearch supports `hash` and `JSON` data type and not all Redis installation necessarily has loaded this module. 
...
All along the way in our Redis implementation, we have employed `string`, `hash` and `sorted set` and `RediSearch` module. Whereas there are still `list`, `set`, `Geo`, `Pub/Sub`, `Stream` and other modules. Those data structures are easy to understand and yet can orchestrate complicated interactivity. 
...
IMHO, MySQL is business-oriented and Redis is noncommercial-oriented. That's why it seems awkward and clumsy in implementing relational *things* such AUTO_INCREMENT. 
