...
Redis is in-memory datastore featuring data structures such as `string`, `list`, `set`, `sorted set`, `hash`... etc, but there no point to implement our own full-text search by reading hash by hash, checking value by value. Although it is completely possible but not effective after all and violates [DRY](https://en.wikipedia.org/wiki/Don%27t_repeat_yourself) principle. 
