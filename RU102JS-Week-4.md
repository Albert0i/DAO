...
- We then get the current time in milliseconds, storing it in `now`. The name of member does not matter provided that it's not duplicated. The score is the paramount here. The line `const member = `${now}-${Math.random()}` is *smoke and mirrors* to captivate the audience while concealing the secrets of the tricks; 
